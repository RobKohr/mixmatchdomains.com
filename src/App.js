...
const Index = () => {
  ...
  function domainPartClick(part) {
    const update = {
      ...quickMixerFormHandler.fields,
      domain: quickMixerFormHandler.fields.domain + part,
    };
  ...
  return (
    <div id="page-index">
      <SearchForm domainPartClick={domainPartClick} />
      <QuickMixer formHandler={quickMixerFormHandler} />
    </div>
  );
};
...
const SearchForm = ({ domainPartClick }) => {
  ...
  const initialFormState = {
    term: '',
    matchType: 'Synonym',
    orderLocation: 'Any Position',
    isRequired: 'Required',
  };
  const fieldGroupDefaultValues = {};
  fieldGroups.forEach(fieldGroup => {
    fieldNumbers.forEach(fieldNumber => {
      initialFormState[`${fieldGroup}${fieldNumber}`] =
        initialFormState[fieldGroup];
    });
  });
  ...
  const formHandler = InitializeForm({
    initialFormState,
    onSubmitCallback,
    onChangeCallback,
  });
  ...
  return (
    <div id="search">
      <form id="terms" onSubmit={formHandler.onSubmit}>
        <Term number={0} formHandler={formHandler} />
        <Term number={1} formHandler={formHandler} />
        <Term number={2} formHandler={formHandler} />
      ...
      <Results
        domainPartClick={domainPartClick}
        tld={formHandler.fields.tld}
        agent={formHandler.fields.agent}
      />
    </div>
  );
};
    ...
    <span className="result-section">
      {r.isAvailable && (
        <span className="register">
          <a href={domainLink(r.domainWithTLD)}>
            Go register {r.domainWithTLD}!
          </a>
        </span>
      )}
      {!r.isAvailable && (
        <span className="register">
          <a href={domainLink(r.domainWithTLD)}>
            Try .net, .org, .it, info, and others &rarr;
          </a>
        </span>
      )}
    </span>
  ...
  return (
    <div id="quickmixer">
      ...
      <TextInput
        name="domain"
        formHandler={formHandler}
        size={formHandler.fields.domain.length}
      />
      ...
      <div class="description">
        Test the availablity of a single domain. You can click on word parts in
        search results to append them to the quick-mixer.
      </div>
    </div>
  );
};
  ...
  return (
    <span className="colorized-domain">
      {domainObj.domainParts.map(function partProcess(part) {
        ...
        return (
          <span class="colorized-domain-part" onClick={partClick}>
            {part}
          </span>
        );
      })}
...
const Term = ({ number, formHandler }) => {
  ...
  return (
    <div className={`term term-${number} ${termClass}`}>
      <TextField
        tabindex="1"
        label={`Term ${number + 1}`}
        name={`term${number}`}
